...
function transPage(cur, next, removed, showSchema) {
  ...
  if (cur) {
    cur.fadeOut(500, () => {
      ...
      if (!removed) GLOBAL.previousContent.push(cur);
    ...
    if (!removed) GLOBAL.previousContent.push(next);

    GLOBAL.currentContent = next;
  }

  if (GLOBAL.customSchema) GLOBAL.customSchema.end();
...
function goBack(index) {
  if (GLOBAL.previousContent.length && GLOBAL.currentContent) {
    index = index === undefined ? GLOBAL.previousContent.length - 1 : index;
  ...
  if (foundIndex !== undefined) {
    try {
      ...
      homebridge.toast.success(
        selectedCar + " removed from config!",
        "Success"
      );
    } catch (err) {
  ...
  try {
    ...
    if (!GLOBAL.pluginConfig.length) {
      ...
      if (
        ...
        (GLOBAL.pluginConfig[0].cars && !GLOBAL.pluginConfig[0].cars.length)
      ) {
        GLOBAL.pluginConfig[0].cars = [];
...
$("#auth").on("click", () => {
  try {
    ...
    let carConfig = GLOBAL.pluginConfig[0].cars.find(
      (car) => car && car.name === GLOBAL.carOptions.name
    );

    if (carConfig) {
      return homebridge.toast.error(
        "There is already a car configured with the same name!",
        "Error"
      );
    } else if (!GLOBAL.carOptions.name) {
      return homebridge.toast.error(
        "There is no name configured for this car!",
        "Error"
      );
    } else if (!GLOBAL.carOptions.clientID) {
      return homebridge.toast.error(
        "There is no client ID configured for this car!",
        "Error"
      );
    } else if (!GLOBAL.carOptions.clientSecret) {
      return homebridge.toast.error(
        "There is no client secret configured for this car!",
        "Error"
      );
    } else if (!GLOBAL.carOptions.vin) {
      return homebridge.toast.error(
        "There is no VIN configured for this car!",
        "Error"
      );
    }
...
$("#startAuth").on("click", async () => {
  try {
    ...
    GLOBAL.carOptions.authorizationUri = await homebridge.request(
      "/authCode",
      GLOBAL.carOptions
    );

    const win = window.open(
      GLOBAL.carOptions.authorizationUri,
      "windowname1",
    ...
    const pollTimer = window.setInterval(function () {
      if (win.document.URL.includes("?code=")) {
        ...
        GLOBAL.carOptions.autherization_code =
          win.document.URL.split("?code=")[1];
...
$("#generateToken").on("click", async () => {
  try {
    ...
    GLOBAL.carOptions.token = await homebridge.request(
      "/authToken",
      GLOBAL.carOptions
    );
...
$("#editCar").on("click", () => {
  ...
  if (!car) return homebridge.toast.error("Can not find the car!", "Error");
...
$("#refreshCar").on("click", async () => {
  if (GLOBAL.customSchema && GLOBAL.carOptions) {
    ...
    let car = GLOBAL.pluginConfig[0].cars.find(
      (car) => car.name === GLOBAL.carOptions.name
    );
...
$("#removeCar").on("click", async () => {
  try {
    ...
    transPage(
      false,
      GLOBAL.pluginConfig[0].cars.length
        ? $("#isConfigured")
        : $("#notConfigured")
    );
  } catch (err) {
